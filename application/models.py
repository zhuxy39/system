...
class major(models.Model):
	#采用django默认创建的自增字段
	college = models.ForeignKey(academy, on_delete=models.DO_NOTHING)				#隶属学院
	major_name = models.CharField(max_length=20, null=False)						#专业名称
...
class teacher(models.Model):
	...
	academy = models.ForeignKey(academy,on_delete=models.DO_NOTHING)				#所处学院
	phone = models.CharField(max_length=20,null=False)								#联系方式
	status = models.BooleanField(null=False,default=True)							#账号状态 有效=1 失效=0 
	create_time = models.DateTimeField(auto_now_add=True)
	class Meta:
...
class staff(models.Model):
	...
	staff_name = models.CharField(max_length=20, null=False, default = "")
...
class device(models.Model):
	...
	responsible_staff = models.ForeignKey(User, on_delete=models.DO_NOTHING) 		#设备负责人的工号：user表的id字段
...
class device_apply(models.Model):
	#主键采用django默认创建的自增字段
	applicant = models.ForeignKey(User,on_delete=models.DO_NOTHING,related_name='device_apply_to_user')		        	#申请人编号
	classroom = models.ForeignKey(classroom,on_delete=models.SET_NULL,related_name='device_apply_classroom',null=True)		    #教室编号
	device_type = models.IntegerField(null=False)									#设备类别_
	...
	apply_head = models.ForeignKey(User,on_delete=models.DO_NOTHING,null=True,related_name='device_apply_apply_head')		#审批人
	responsible_staff = models.ForeignKey(User,on_delete=models.DO_NOTHING,null=True,related_name='device_apply_responsible_staff')
...
class classroom_apply(models.Model):
	#主键采用django默认创建的自增字段
	applicant = models.ForeignKey(User,on_delete=models.DO_NOTHING,related_name='classroom_apply_applicant')			        #申请人编号
	classroom = models.ForeignKey(classroom,on_delete=models.SET_NULL,related_name='classroom_apply_classroom',null=True)		    #教室编号
	use_date = models.DateField(null=False)											#需求日期
	...
	apply_head_1 = models.ForeignKey(User,on_delete=models.DO_NOTHING,null=True,related_name='classroom_apply_apply_head_1')    #审批人1
	apply_head_2 = models.ForeignKey(User,on_delete=models.DO_NOTHING,null=True,related_name='classroom_apply_apply_head_2')    #审批人2
	...
	responsible_teacher = models.ForeignKey(teacher,on_delete=models.DO_NOTHING,null=False,related_name='classroom_apply_responsible_teacher') #申请问责老师
	class Meta:
		ordering = ['apply_time']
	
class classroom_apply_record(models.Model):
	#主键采用django默认创建的自增字段
	apply = models.ForeignKey(classroom_apply,on_delete=models.DO_NOTHING,related_name='classroom_apply_record_apply')			#这件教室的使用对应了哪张申请单
	classroom = models.ForeignKey(classroom,on_delete=models.CASCADE,null=False,related_name='classroom_apply_record_classroom')	#教室编号
	use_date = models.DateField(null=False)											#需求日期
	apply_section_begin = models.IntegerField(null=False, default = 0)				#需求节数起点
	apply_section_end = models.IntegerField(null=False, default = 0)				#需求节数终点	
	class Meta:
...
class curriculum(models.Model):
	...
	classroom = models.ForeignKey(classroom,on_delete=models.CASCADE,related_name='curriculum_classroom') 			#课室编号
...
class device_broken(models.Model):
	...
	device = models.ForeignKey(device,on_delete=models.DO_NOTHING,related_name='device_broken_device') 					#设备
	...
	pass_user = models.ForeignKey(User,null = True, on_delete=models.DO_NOTHING,related_name='device_broken_pass_user')				#故障处理通过人
	class Meta:
		ordering = ['submit_time']

class device_apply_record(models.Model):
	#主键采用django默认创建的自增字段
	apply = models.ForeignKey(device_apply,on_delete=models.DO_NOTHING,related_name='device_apply_record_apply')				#对应哪一张设备申请单
	device = models.ForeignKey(device,on_delete=models.DO_NOTHING,related_name='device_apply_record_device')						#设备
