...
def add_device(request):
	...
	new_device.is_stable = True if stable == '1' else False
	new_device.save()
...
def detail_info(request,apply_id):
	...
	#初步符合条件的设备
	device_infos = device.objects.filter(status=True,is_stable=True,device_type=type,device_name=name)
	help()
...
def add_classroom(request, classroom_id):
	if request.user.is_authenticated and request.user.is_superuser:
		...
		category = request.GET.get('classroom_category')
		...
		if count > 0:
			...
			c = classroom(classroom_id = classroom_id, size = classroom_size, category = category, status = False)
			c.save()
			if category == 0:
				category = '普通课室'
			elif category == 1:
...
def classroom_detail(request, classroom_id):
	if request.user.is_authenticated and request.user.is_superuser:
		...
		applies = classroom_apply.objects.filter(classroom = classroom_id, apply_status__in = [1,2], use_date__gte = today, use_date__lte = later )
		for apply in applies:
			...
			data.append(content)
		courses = curriculum.objects.filter(classroom = classroom_id)
		for course in courses:
			delta = course.week_day -1 - weekday
			if delta < 0 :
				content = [course.section_begin, course.section_end, delta+7, 0, course.course_name]
				data.append(content)
				value = [course.section_begin, course.section_end, delta+14, 0, course.course_name]
				data.append(content)
			else:
				value = [course.section_begin, course.section_end, delta, 0, course.course_name]
				data.append(content)
				value = [course.section_begin, course.section_end, delta+7, 0, course.course_name]
				data.append(content)
		context = {}
		context['data'] = data
...
def classroom_device(request, classroom_id):
	if request.user.is_authenticated and request.user.is_superuser:
		pageIndex = int(request.GET.get('pageIndex'))
		pageSize = int(request.GET.get('pageSize'))
		devices = device.objects.filter(storage_location = classroom_id, if_stable = True)
		device_list = []
		device_list_count = devices.count()
		if devices:
			devices = devices.values_list('device_type', 'device_name').annotate(device_count = Count('device_id'))
			devices = devices[pageSize*(pageIndex-1) : pageSize*pageIndex]
			for d in devices:
				content = {}
				content['device_type'] = d[0]
...
def apply_classroom_list_1(request): #展示待初审的申请表单
	if request.user.is_authenticated and request.user.is_superuser:
		...
		if search_word == 'apply_id':
			applies = applies.filter(id = int(search_word))
		elif search_word == 'applicant':
			u = User.
			applies = applies.filter(applicant = search_word)
		elif search_word == 'apply_date':
			applies = applies.filter(ause_date__contains = search_word)
		apply_list = []
		...
		if applies:
			for apply in applies:
				...
				content['applicant_id'] = apply.applicant
...
def apply_classroom_list_2(request): #展示待复审的申请表单
	if request.user.is_authenticated and request.user.is_superuser:
		...
		if search_word == 'apply_id':
			applies = applies.filter(id__contains = search_word)
		elif search_word == 'applicant':
			applies = applies.filter(applicant__contains = search_word)
		elif search_word == 'apply_date':
			applies = applies.filter(use_date__contains = search_word)
		apply_list = []
		apply_list_count = applies.count()
		if applies:
			for apply in applies:
				content = {}
				content['classroom_apply_id'] = apply.classroom_apply_id
				content['applicant_id'] = apply.applicant_id
				content['apply_time'] = apply.apply_time
				content['apply_date'] = apply.use_date
				content['apply_section_begin'] = apply.apply_section_begin
				content['apply_section_end'] = apply.apply_section_end
				content['apply_time'] = apply.apply_time
				content['apply_size'] = apply.apply_size
				content['apply_head_1'] = apply.apply_head_1
...
def refuse_classroom_apply(request, classroom_apply_id):
	if request.user.is_authenticated and request.user.is_superuser:
		apply = classroom_apply.objects.get(id = classroom_apply_id)
		reason = request.GET.get('refuse_reason')
		if apply.apply_status == 0:
			apply.apply_status = 3
			apply.apply_head_1 = request.user.username
			apply.pass_time_1 = timezone.now()
			apply.refuse_reason = reason
			apply.save()
		elif apply.apply_status == 1:
			apply.apply_status = 3
			apply.apply_head_2 = request.user.username
			apply.pass_time_2 = timezone.now()
			apply.refuse_reason = reason
			apply.save()
		return JsonResponse({
			'code' : '200',
...
def choose_available_classroom(request, classroom_apply_id):
	if request.user.is_authenticated and request.user.is_superuser:
		...
		full_classrooms_1 = classroom_apply.objects.filter(apply_status__in = [1,2], apply_section_end__gte = apply_section_begin, use_date = apply_date)
		full_classrooms_1 = full_classrooms_1.values('classroom_id').distinct()
		full_classrooms_2 = classroom_apply.objects.filter(apply_status__in = [1,2], apply_section_begin__lte = apply_section_end, use_date = apply_date)
		full_classrooms_2 = full_classrooms_2.values('classroom_id').distinct()
		full_classrooms = full_classrooms_1 | full_classrooms_2
		available_classrooms_count = 0
		available_classrooms = []
		
		classrooms = classroom.objects.filter(size__gte = apply_size, category__gte =  apply_category)
		for room in full_classrooms:
			classrooms = classrooms.exclude(classroom_id = room.classroom_id)
		for room in classrooms:
			...
			content['classroom_category'] = room.category
			avaliable_classrooms.append(content)
...
def pass_classroom_apply_1(request, classroom_apply_id):
	if request.user.is_authenticated and request.user.is_superuser:
		...
		apply = classroom_apply.objects.get(id = apply_id)
		apply.classroom_id = classroom_id
		apply.apply_head_1 = request.user.username
...
def pass_classroom_apply_2(request, classroom_apply_id):
	if request.user.is_authenticated and request.user.is_superuser:
		...
		apply.apply_status = 2
		record = classroom_apply_record(apply=apply.id, classroom=apply.classroom_id, use_date = apply.use_date, apply_section_begin = apply.apply_section_begin,apply_section_end = apply.apply_section_end)
		record.save()
		apply.save()
		return JsonResponse({
			'classroom_apply_id' : classroom_apply_id,
...
def classroom_apply_reason(request, classroom_apply_id):
	if request.user.is_authenticated and request.user.is_superuser:
		reason = classroom_apply.objects.get(id = classroom_apply_id).apply_reason
		return JsonResponse({
			'status' : 'success',
			'code' : '200',
			'message' : '请求成功',
...
def apply_classroom_record(request):
	if request.user.is_authenticated and request.user.is_superuser:
		...
		for apply in applies:
			...
			content['apply_head_1'] = apply.apply_head_1
			content['pass_time_1'] = apply.pass_time_1
			content['apply_head_2'] = apply.apply_head_1
			content['pass_time_2'] = apply.pass_time_1
			if apply.apply_status == 0 :
